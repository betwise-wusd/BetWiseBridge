...
public class AESUtils2 {
     ...
     * android 9.0适配
     * */
    private static byte[] getSecretKeyEncoded(String content) throws Exception {
        //SecretKey skey = BrokenKeyDerivationActivity.deriveKeyInsecurely(content,16);
        byte[] passwordBytes = content.getBytes(StandardCharsets.UTF_8);
        SecretKey skey = new SecretKeySpec(
                InsecureSHA1PRNGKeyDerivator.deriveInsecureKey(passwordBytes, 16),
                "AES");
        byte[] raw = skey.getEncoded();
        return raw;
    }
...
//            return Base64Encoder.encode(result);
            return new String(Base64.encode(result, Base64.DEFAULT), StandardCharsets.UTF_8);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
...
//            return Base64Encoder.encode(result);
    ...
    private static byte[] encrypt(String key, byte[] clear) throws Exception {
        int sdk_version = android.os.Build.VERSION.SDK_INT;
        byte[] raw = sdk_version > 27 ? getSecretKeyEncoded(key) : getRawKey(key.getBytes(StandardCharsets.UTF_8));
